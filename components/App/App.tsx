...
const App = () => {
    ...
    useEffect(() => {
        ...
        // Patch requestPointerLock to handle "too frequent" errors from browser
                ...
                try {
                    ...
                    if (result instanceof Promise) {
                        return result.catch((err: any) => {
                            ...
                            if (msg.includes('immediately') || err.name === 'NotAllowedError' || msg.includes('gesture')) {
                                return; // Silent fix
                            }
                            console.warn('Pointer lock internal error:', err);
                        });
                    }
                    return result;
                } catch (err: any) {
                    const msg = String(err.message || err || '');
                    if (msg.includes('immediately') || err.name === 'NotAllowedError') {
                    ...
                    <div style={{ display: 'flex', gap: Tokens.spacing.sm, alignItems: 'center' }}>
                        ...
                        <div style={{
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span style={{ color: Tokens.colors.contentDim, fontSize: '10px', fontFamily: Tokens.fonts.body, textTransform: 'uppercase', letterSpacing: '1px' }}>HULL INTEGRITY</span>
                                <span style={{ color: Tokens.colors.content, fontSize: '12px', fontFamily: Tokens.fonts.data }}>{Math.ceil(playerHp)}%</span>
                            </div>
                            <div style={{ width: '100%', height: '4px', background: Tokens.colors.surfaceLight, borderRadius: '2px', overflow: 'hidden' }}>
                                <motion.div 
                                    initial={{ width: '100%' }}
                                    animate={{ width: `${playerHp}%`, backgroundColor: playerHp > 30 ? Tokens.colors.accent : '#FF0000' }}
                                    style={{ height: '100%', boxShadow: playerHp < 30 ? '0 0 10px #FF0000' : 'none' }}
                                />
                            </div>
                        </div>
                    </div>
