...
export class Environment {
  ...
  constructor() {
    ...
    // Simple Flat Terrain
    const segs = 1; 
    this.heightMapSize = segs;
    const terrainData = createTerrainMesh(400, 400, segs, segs, [0.25, 0.45, 0.2], null); // Pass null for flat
    this.floor = terrainData.mesh;
    
    this.heights = new Float32Array((segs + 1) * (segs + 1));
    this.heights.fill(0);

    // Add flat collision mesh to Jolt
    gfx3JoltManager.addBox({
        width: 400, height: 2, depth: 400,
        x: 0, y: -1.0, z: 0, // Surface at y=0
        motionType: Gfx3Jolt.EMotionType_Static,
        layer: JOLT_LAYER_NON_MOVING
    });

    // Add a VERY thick foundation floor as ultimate fallback for tunneling (deep enough)
    gfx3JoltManager.addBox({
        width: 2000, height: 20, depth: 2000,
        x: 0, y: -15, z: 0, // Top surface is at -5.0 (Deep enough below terrain)
        motionType: Gfx3Jolt.EMotionType_Static,
        layer: JOLT_LAYER_NON_MOVING
    });
