...
export function createTerrainMesh(width: number, depth: number, segmentsW: number, segmentsD: number, color: [number, number, number], heightmapCanvas: HTMLCanvasElement | null): { mesh: Gfx3Mesh, vertices: Array<number>, indexes: Array<number> } {
  const mesh = new Gfx3Mesh();
  mesh.setTag(0, 0, Gfx3MeshEffect.PIXELATION); 
  ...
  if (heightmapCanvas) {
    ...
    imgData = ctx.getImageData(0, 0, heightmapCanvas.width, heightmapCanvas.height).data;
  }
  ...
  for (let z = 0; z <= segmentsD; z++) {
    ...
    for (let x = 0; x <= segmentsW; x++) {
      ...
      if (imgData && heightmapCanvas) {
          ...
          heightVal = imgData[idx] / 255.0; // 0.0 to 1.0
      }
      
      const py = heightVal > 0 ? (heightVal * 30.0 - 0.5) : 0; 
